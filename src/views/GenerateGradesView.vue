...
<template>
  <main>
    ...
    <div class="grade-data">
      <p>
        Estimated CGPA: <span>{{ studentCGPA }}</span>
      </p>
      <p>
        Total Number of Units: <span>{{ store.numberOfUnits }}</span>
      </p>
      <p>
        Current CGPA: <span>{{ store.studentCGPA }}</span>
      </p>
    </div>
    <div class="tables">
      <template v-for="semester in store.courseStructure" :key="semester.id">
        <GradeTable
          :title="semester.title"
          :courses="semester.courses"
          v-if="semester.estimate"
          @grade-change="
            (totalGradePoints, totalUnits) =>
              calculateCGPA(totalGradePoints, totalUnits)
          "
        />
      </template>
    </div>
